...
const { seedTestData, cleanupTestData, getAdminToken, getHeadToken, getMemberToken, getTestMemberId, getTestFamilyId } = require('../helpers/setup');
const { getDb, setActiveDb } = require('../../backend/db/db');
...
describe('Customers API', () => {
  ...
  describe('PUT /api/customers/:id', () => {
    it('should update an existing customer using dynamic ID', async () => {
      ...
      expect(res.body.data.shortName).toBe('测');
    });
    ...
    it('should update name only when shortName is not provided', async () => {
      ...
      expect(updateRes.body.data.shortName).toBe('部');
    });
    ...
    it('should update shortName only when name is not provided', async () => {
      ...
      expect(updateRes.status).toBe(200);
      expect(updateRes.body.data.shortName).toBe('新简');
    ...
    it('should update family_name when customer name is updated', async () => {
      ...
      const db = getDb(TEST_DB_NAME);
      const family = db.prepare('SELECT * FROM families WHERE head_id = ?').get(getTestMemberId());
